...
import math
import torch # dependency with slice operation
...
def apply_scaling(freqs: torch.Tensor, scale_factor: float, orig_context_len: int, device=None):
    low_freq_factor = 1
    high_freq_factor = 4
    ...
    return ttnn.Tensor(shape=[len(new_freqs)], dtype=ttnn.float32, device=device)
...
def precompute_freqs(dim: int, end: int, theta, scale_factor, orig_context_len, device=None):
    ...
    if scale_factor is not None:
        ...
        freqs = ttnn.Tensor(shape=[len(freqs)], dtype=ttnn.float32, device=device)
