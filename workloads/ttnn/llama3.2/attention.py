...
import math
import torch # this dependency yet to be resolved (keeping it because of slice operations)
...
class Attention():
    def __init__(
        ...
        layer_name = f"{__class__}_{__name__}_{layer_num}"
        ...
        for i in range(self.num_devices_per_group):
            wq_selected = ttnn._rand((3072, 3072), device=self.mesh_device, dtype=ttnn.bfloat16)
            wk_selected = ttnn._rand((1024, 3072), device=self.mesh_device, dtype=ttnn.bfloat16)
            wv_selected = ttnn._rand((1024, 3072), device=self.mesh_device, dtype=ttnn.bfloat16)
        ...
        wo_str_weight = ttnn._rand((3072, 3072), device=self.mesh_device, dtype=ttnn.bfloat16)
    ...
    def forward_decode(
        ...
        paged_update_cache(keys, k_heads_1BKD, update_idxs_tensor=current_pos, page_table=page_table)
        paged_update_cache(values, v_heads_1BKD, update_idxs_tensor=current_pos, page_table=page_table)
        ...
        if page_table:
            ...
            attn_output_1G4D = scaled_dot_product_attention_decode(
                q_heads_1BQD,
                keys,
                values,
                cur_pos_tensor=current_pos,
        ...
        attn_output_cat = nlp_concat_heads_decode(
            attn_output_11BH,
            num_heads=self.n_local_heads,
        )
    ...
    def forward_prefill(
        ...
        (
            ...
            v_heads_1VSD,
        ) = nlp_create_qkv_heads( #ttnn.experimental.nlp_create_qkv_heads(
            xqkv_fused,
            num_heads=self.n_local_heads,
            num_kv_heads=self.n_local_kv_heads,
        ...
        q_heads_1QSD = rotary_embedding_llama(
            q_heads_1QSD_pre_rot,
            rot_mats[0],
            rot_mats[1],
        ...
        k_heads_1KSD = rotary_embedding_llama(
            k_heads_1KSD_pre_rot,
            rot_mats[0],
            rot_mats[1],
            self.transformation_mats["prefill"],
        ...
        attn_output_84SD = scaled_dot_product_attention(
            q_heads_1QSD_8b,
            k_heads_1KSD_8b,
            v_heads_1VSD_8b,
            is_causal=True,
        ...
        # Output matmul
        ###
        attn_output_11SH = nlp_concat_heads(
            attn_output_1QSD,
            memory_config=ttnn.DRAM_MEMORY_CONFIG,
        )
