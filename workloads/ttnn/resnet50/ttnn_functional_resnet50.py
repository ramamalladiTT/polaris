...
def ResnetLinear(
    ...
    weight        = weight.reshape(ttnn.Shape(weight.shape).to_rank(4)) #type: ignore
    bias          = bias.reshape  (ttnn.Shape(bias.shape).to_rank(4))   #type: ignore
...
class resnet50Bottleneck:
    ...
    def __call__(
        ...
        if eltwise_binary_out_in_place:
            ...
            #TODO: add_ is inplace binary operation: A = A + B 
            #out = ttnn.add_(
            #    out,
            #    ds_out,
            ...
            out = ttnn.add(
                out,
                ds_out,
                #TODO: add support for activations, memory_config
                #activations=[ttnn.UnaryWithParam(ttnn.UnaryOpType.RELU)],
...
class resnet50:
    ...
    def run(self, input_tensor, device, ops_parallel_config) -> ttnn.Tensor:
        ...
        if is_blackhole():
            ...
            core_range_set = ttnn.CoreRangeSet({ ttnn.CoreRange( ttnn.CoreCoord(0, 0), ttnn.CoreCoord(8, 7) ) })
