...
from workloads.ttnn.resnet50.utils import _nearest_y, is_blackhole, is_grayskull, is_wormhole_b0


#SK: NOIMPL MatmulMultiCoreReuseMultiCast1DProgramConfig
...
def ResnetLinear(
    ...
    weight        = weight.reshape(ttnn.Shape(weight.shape).to_rank(4))
    bias          = bias.reshape  (ttnn.Shape(bias.shape).to_rank(4))
...
class resnet50:
    def __init__(
        ...
        self.layer1 = self._make_layer(parameters=parameters.layer1, planes= 64, blocks=layers[0], stride=1, model_config=model_config,)
        self.layer2 = self._make_layer(parameters=parameters.layer2, planes=128, blocks=layers[1], stride=2, model_config=model_config,)
        self.layer3 = self._make_layer(parameters=parameters.layer3, planes=256, blocks=layers[2], stride=2, model_config=model_config,)
        self.layer4 = self._make_layer(parameters=parameters.layer4, planes=512, blocks=layers[3], stride=2, model_config=model_config,)
        ...
        conv_dummy_tensor = ttnn._rand((self.fold_output_shape), dtype=ttnn.bfloat16, device=device)
...
if __name__ == '__main__':
    ...
    parameters   = create_rn50_params(device=device, dtype=ttnn.bfloat16)
